    ...
    *,
    ...
    for sequence_category in pbar:
        ...
        for idx, sequence_name in enumerate(pbar_inner):
            ...
            imagePath = output_dir / 'image' / sequence_category / f'{sequence_name}.png'
            filepath.parent.mkdir(exist_ok=True, parents=True)
            imagePath.parent.mkdir(exist_ok=True, parents=True)
            io.save_pointcloud(data=output, path=str(filepath))
            diffusion_utils.visualize_pointcloud_batch_pytorch3d(pointclouds=output, 
                    output_file_image=imagePath, cameras=camera)
